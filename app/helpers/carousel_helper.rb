module CarouselHelper
  ...
  class Carousel
    ...
    def slide_tag(image, is_active)
      ...
      content_tag(:div, image_tag(image, :class => "img-fluid"), options)
    end
    ...
    def control_tag(direction)
      ...
      span = content_tag(:span, '',class: "icon-"+ direction == 'left' ? 'prev' : 'next' , aria:{hidden:"true"}) 
      span2 = content_tag(:span, direction == 'left' ? 'Previous' : 'Next',class:'sr-only')
      
      control = link_to(safe_join([span,span2]), "##{uid}", options)
    end
  end
end